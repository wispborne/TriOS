...
/// - see [DataTable3.smRatio], [DataTable3.lmRatio] and same properties on [PaginatedDataTable2].
...
@immutable
class DataRow3 extends DataRow {
  ...
  /// Decoration to nbe applied to the given row. When applied, it [DataTable3.dividerThickness]
  /// won't take effect
...
class DataCell3 extends DataCell {
  ...
  const DataCell3(
    ...
    super.onTapDown,
    super.onTapCancel,
    Widget Function(BuildContext, Widget)? this.builder,
  });
}
...
/// By using [DataColumn3] instead of [DataColumn] it is possible to control
/// relative column sizes (setting them to S, M and L). [DataRow3] provides
...
class DataTable3 extends DataTable {
  ...
  Widget _buildDataCell({
    ...
    required Widget label,
    required Function(BuildContext, Widget)? builder,
    required bool numeric,
            ...
            for (int dataColumnIndex = 0;
              ...
              for (final DataRow row in rows) {
                ...
                var c = _buildDataCell(
                    ...
                    padding: padding,
                    specificRowHeight:
                        row is DataRow3 ? row.specificRowHeight : null,
                    label: cell.child,
                    builder: cell is DataCell3 ? cell.builder : null,
                    numeric: column.numeric,
                    ...
                    onRowTap: row is DataRow3 ? row.onTap : null,
                    onRowDoubleTap: row is DataRow3 ? row.onDoubleTap : null,
                    onRowLongPress: row.onLongPress,
                    onRowSecondaryTap:
                        row is DataRow3 ? row.onSecondaryTap : null,
                    onRowSecondaryTapDown:
                        row is DataRow3 ? row.onSecondaryTapDown : null,
    ...
    if (displayCheckboxColumn) {
      ...
      for (final DataRow row in rows) {
        var x = _buildCheckbox(
            ...
            onRowTap: () {
              if (row is DataRow3 && row.onTap != null) {
                row.onTap?.call();
              } else {
            ...
            tristate: false,
            rowHeight: rows[rowIndex] is DataRow3
                ? (rows[rowIndex] as DataRow3).specificRowHeight
                : null);
  ...
  List<double> _calculateDataColumnSizes(BoxConstraints constraints,
    ...
    var totalFixedWidth = columns.fold<double>(
        ...
        (previousValue, element) =>
            previousValue +
            (element is DataColumn3 && element.fixedWidth != null
                ? element.fixedWidth!
                : 0.0));
    ...
    final widths = List<double>.generate(columns.length, (i) {
      ...
      if (!(column is DataColumn3 && column.fixedWidth != null)) {
    ...
    for (var i = 0; i < widths.length; i++) {
      ...
      if (!(columns[i] is DataColumn3 &&
          (columns[i] as DataColumn3).fixedWidth != null)) {
        widths[i] *= ratio;
      }
    }
      ...
      (int index) {
        ...
        if (headingRow != null && index == 0) {
          ...
          Decoration? rowDecoration =
              rows[rowStartIndex + actualIndex] is DataRow3
                  ? (rows[rowStartIndex + actualIndex] as DataRow3).decoration
                  : null;
