...
import 'package:trios/thirdparty/pluto_grid_plus/lib/src/manager/state/grid_state.dart';
...
class _Smol3State extends ConsumerState<Smol3>
  ...
  // final List<Mod> _internalGridMods = [];
  ...
  void initState() {
    ...
    final versionCheckResults =
        ref
            .read(AppState.versionCheckResults)
            .valueOrNull;
  ...
  Widget build(BuildContext context) {
    ...
    if (!isChangingModProfileProvider) {
      if (stateManager != null) {
        ...
        updateRows(stateManagerNotNull, gridState!, enabledMods, disabledMods);
  ...
  Mod? _getModFromKey(Key? key) {
    return key is ValueKey<RowKey>
        ? allMods.firstWhereOrNull((m) => m.id == key.value.modId)
        : null;
  }

  RowKey? _getRowKeyFromKey(Key? key) {
    return key is ValueKey<RowKey> ? key.value : null;
  }

  void updateRows(PlutoGridStateManager stateManager, ModsGridState gridState,
      List<Mod> enabledMods,
      List<Mod> disabledMods) {
    final notifyOnEach = true;

    // TODO you were realizing that `stateManager.refRows.originalList` includes the row groups and doesn't include hidden rows
    // and maybe you should just roll with that.
    final isEnabledShown = gridState.isGroupEnabledExpanded;
    final isDisabledShown = gridState.isGroupDisabledExpanded;
    final newModList = [
      null,
      ...(isEnabledShown ? enabledMods : []),
      null,
      ...(isDisabledShown ? disabledMods : [])
    ];

    final oldEnabledMods = stateManager.refRows.originalList
        .map((row) => _getModFromKey(row.key))
    .whereNotNull()
        .toList();

    // Do equalityChecker comparison using short circuiting to avoid unnecessary work
    final diffEnabled = calculateListDiff(oldEnabledMods, enabledMods,
        equalityChecker: (a, b) => a?.id == b?.id && a.hashCode == b.hashCode);


    final updates = time(() => diffEnabled.getUpdatesWithData());
    ...
    for (final update in updates) {
      update.when(
        insert: (pos, data) {
          stateManager.appendRows([createRow(data)!], notify
          :
          notifyOnEach
          );
        },
        remove: (pos, data) {
          Fimber.d('remove $pos $data');
          // final modRemoved = _internalGridMods.removeAt(pos);
          stateManager.removeRows(
              [stateManager.refRows.filteredList.elementAt(pos)],
              notify: notifyOnEach);
        },
        change: (pos, oldData, newData) {
          Fimber.d('change on $pos from $oldData to $newData');
          final oldRow = stateManager.refRows.originalList.elementAt(pos)!;
          final newCells = createRow(newData)!.cells.values.toList();
          // _internalGridMods[pos] = newData;
        ...
        move: (from, to, data) {
          Fimber.d('move $data from $from to $to');
          // _internalGridMods.insert(to, _internalGridMods.removeAt(from));
          final row = stateManager.refRows.originalList.elementAt(from);
          stateManager.moveRowsByIndex([row], to, notify: notifyOnEach);
        },
      );
    }
    ...
    //           final prevRowKey = _getRowKeyFromKey(oldRow.key);
    //           if (prevRowKey?.modHashCode == mod.hashCode) {
    //             return null;
    //           }
    ...
    //
    //   // Update row hashcode
    //   (oldRow.key as ValueKey<RowKey>).value.modHashCode = newRow.hashCode;
    //
    //   oldRow.cells.entries.forEachIndexed((index, entry) {
    //     String columnField = entry.key;
    //     final newValue = newCells[index].value;
  ...
  List<PlutoColumn> createColumns(double versionSelectorWidth,
                            ...
                            tooltipWidget: TooltipFrame(
                                child: Column(
                                  ...
                                  children: [
                                    // bold
                                    Text(
                                        "Estimated VRAM use by ${rendererContext
                                            .cell.value} mods\n",
                                        style: Theme
                                            .of(context)
                                            .textTheme
                                    ...
                                    if (graphicsLibConfig != null &&
                                      ...
                                      Text(
                                          "Normal maps: ${graphicsLibConfig
                                              .areGfxLibNormalMapsEnabled
                                              ? "on"
                                              : "off"}"
                                              "\nMaterial maps: ${graphicsLibConfig
                                              .areGfxLibMaterialMapsEnabled
                                              ? "on"
                                              : "off"}"
                                              "\nSurface maps: ${graphicsLibConfig
                                          ...
                                          Theme
                                              ...
                                              .labelLarge),
                                    Text(
                                        "\n${vramModsNoGraphicsLib
                                            .bytesAsReadableMB()} added by mods (${allEstimates
                                            .map((e) => e.images.length)
                                            .sum} images)"
                                            "${vramFromGraphicsLib.sum() > 0
                                            ? "\n${vramFromGraphicsLib
                                            .sum()
                                            .bytesAsReadableMB()} added by your GraphicsLib settings (${vramFromGraphicsLib
                                            .length} images)"
                                            : ""}"
                                            "${vramFromVanilla != null
                                            ? "\n${vramFromVanilla
                                            .bytesAsReadableMB()} added by vanilla"
                                            : ""}"
                                            "\n---"
                                            "\n${(vramModsNoGraphicsLib +
                                            vramFromGraphicsLib.sum() +
                                            (vramFromVanilla ?? 0.0))
                                            .bytesAsReadableMB()} total",
                                        style: Theme
                                            .of(context)
                                            .textTheme
                                            .labelLarge)
                            ...
                            children: [
                              ...unmetDependencies.map((checkResult) {
                                ...
                                return Padding(
                                  ...
                                  child: Tooltip(
                                    message:
                                    "Requires ${checkResult.dependency
      ...
      PlutoColumn(
        ...
        renderer: (rendererContext) =>
            Builder(builder: (context) {
              ...
              final remoteVersionCheck = versionCheckComparison
                  ?.remoteVersionCheck;
                      ...
                      return Expanded(
                        child: MovingTooltipWidget.text(
                          message: vramEstimate == null
                              ...
                              "\n${withoutGraphicsLib
                              ?.sum()
                              .bytesAsReadableMB()} from mod (${withoutGraphicsLib
                              ?.length} images)"
                              "\n${fromGraphicsLib
                              ?.sum()
                              .bytesAsReadableMB()} added by your GraphicsLib settings (${fromGraphicsLib
                              ?.length} images)"
                              "\n---"
                              "\n${vramEstimate.bytesUsingGraphicsLibConfig(
                              graphicsLibConfig).bytesAsReadableMB()} total",
                          child: Stack(
                            children: [
                              Align(
                                alignment: Alignment.centerLeft,
              ...
              return ContextMenuRegion(
                  ...
                  child: RowItemContainer(
                    child: Opacity(
                      opacity: lightTextOpacity,
                      child: Text(
                          rendererContext.cell.value ?? "(no game version)",
  ...
  PlutoRow? createRow(Mod mod) {
    ...
    final satisfiableDependencies = modCompatibility?.dependencyChecks
        ...
        0;
    return PlutoRow(
      key: ValueKey(RowKey(mod.id, mod.hashCode)),
  ...
  Padding showCompatibilityErrorMessage(DependencyCheck? dependencies,
      String? gameVersion, ThemeData theme) {
    return Padding(
      ...
      child: Builder(builder: (context) {
        return Text(
          dependencies?.gameCompatibility == GameCompatibility.incompatible
              ...
              .map((e) =>
          switch (e.satisfiedAmount) {
            Satisfied _ => null,
            Missing _ =>
            "${e.dependency.formattedNameVersion} is missing.",
            Disabled _ => null,
            VersionInvalid _ =>
            "Missing version ${e.dependency.version} of ${e.dependency
                .nameOrId}.",
            VersionWarning version =>
            "${e.dependency.nameOrId} version ${e.dependency
                .version} is wanted but ${version.modVariant!
                .bestVersion} may work.",
          })
              .join(" • ") ??
              "",
...
@MappableClass()
class RowKey with RowKeyMappable {
  String modId;
  int modHashCode;

  RowKey(this.modId, this.modHashCode);

  @override
  bool operator ==(Object other) => other is RowKey && other.modId == modId;

  @override
  int get hashCode => modHashCode;
}
