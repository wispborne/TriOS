...
4. A single constant (`_semanticColorStrategy`) controls which strategy is active.
...
- Switching `_semanticColorStrategy` between `fromSeed` and `tonalPalette` produces visually distinct but functionally equivalent results.
