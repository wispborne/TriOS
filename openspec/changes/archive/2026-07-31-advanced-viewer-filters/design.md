...
**The track is not to scale.** It runs over positions in `stops` — one notch
per number that exists in the data, all the same width — rather than over the
numbers themselves. Vanilla ship stats are far too lopsided for a to-scale
track:

| Stat | min | max | distinct | median |
|---|---|---|---|---|
| Hull | 30 | 10,000,000 | 52 | 5,000 |
| Flux capacity | 0 | 100,000 | 55 | 2,500 |
| Armor | 0 | 10,000 | 41 | 600 |
| Deployment points | 1 | 600 | 31 | 8 |

Hull's ten million is the DEM Drone, which holds both flux records too. Spaced
by value, every real warship would sit in the first 0.2% of the track — you
couldn't pick "8,000 to 12,000 hull" with a mouse. Drop the drone and it's
still 3%, and mods only widen the gap. One notch per real value keeps all of
it reachable, and the giant outlier is simply the last notch.

`RangeFilterGroup.stopIndexFor(value)` (binary search) maps a value to its
notch; `snapTo` is that plus a lookup. Handle positions are rounded as you
drag, so the handles step from one real value to the next. The readout and the
end labels always show real numbers. `RangeSlider.divisions` is left unset —
it would draw a tick per notch, which is a solid line once a stat has hundreds
of distinct values.
...
One more controller helper: `updateRanges(items)` refreshes every range
group's min/max. It's called with the full item list when that list changes,
so slider ends don't move as you filter. "Clear All" resets sliders along with
everything else through the existing `clearAll()`.
