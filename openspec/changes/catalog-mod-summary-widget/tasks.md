...
- [x] Create `lib/catalog/widgets/mod_summary/mod_summary_data.dart` with `ModSummaryData`, `fromScraped()`, and `fromDetails()` factories (port the merge logic from `ForumPostHeaderData`)
...
- [x] Wrap the scraped mod card content in `MovingTooltipWidget.framed` showing `ModSummaryWidget` with the `tooltip` config
...
- [x] Update `scraped_mod_details_dialog.dart` to build `ModSummaryData.fromScraped(...)` and slim `_Body` (drop image + AI paragraph now shown in the header; keep author text / empty-state)
...
- [x] `fvm flutter analyze` clean (no new issues; two pre-existing warnings in `scraped_mod_card.dart` remain)
