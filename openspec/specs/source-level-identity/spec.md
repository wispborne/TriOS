...
Each `ModRecordSource` subtype that originates name or author information SHALL store that data directly. `InstalledSource` SHALL have `name` and `author` fields (from mod_info.json). `CatalogSource` SHALL have `name` and `authors` (`List<String>?`) fields.
