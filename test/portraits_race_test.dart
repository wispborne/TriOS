...
void main() {
  ...
  test('removeReplacement fails if called multiple times in rapid succession', () async {
    final notifier = container.read(portraitReplacementsProvider.notifier);
    ...
    var state = await container.read(portraitReplacementsProvider.future);
    ...
    // Check final state
    state = await container.read(portraitReplacementsProvider.future);
    ...
    expect(state.containsKey('hash1'), isFalse, reason: 'hash1 should be removed');
    expect(state.containsKey('hash2'), isFalse, reason: 'hash2 should be removed');
    expect(state.containsKey('hash3'), isFalse, reason: 'hash3 should be removed');
  });
}
