...
''';
  ...
  group('loadYamlStream performance', () {
    test('multi-document stream (10 documents)', () {
      final buf = StringBuffer();
      for (var i = 1; i <= 10; i++) {
        buf.writeln('---');
        buf.writeln('id: $i');
        buf.writeln('name: document_$i');
        buf.writeln('tags: [tag_a, tag_b, tag_$i]');
      }
      final yaml = buf.toString();

      const iterations = 200;
      YamlList? result;
      _benchmark('multi-document stream', iterations, () {
        result = loadYamlStream(yaml);
      });

      expect(result, isNotNull);
      expect(result!.length, 10);
      expect(result!.first, isA<YamlMap>());
    });
  });

  group('loadYamlDocument performance', () {
...
''';
      const iterations = 500;
      YamlDocument? result;
      _benchmark('document with directive', iterations, () {
        result = loadYamlDocument(yaml);
      });

      expect(result, isNotNull);
      expect(result!.contents, isA<YamlMap>());
      expect((result!.contents as YamlMap)['application'], 'TriOS');
    });
  });
}
